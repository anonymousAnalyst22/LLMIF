...
{
  // JC update: manually generate the packet
  uint16 len = 13 + aff->asduLength;
  uint8 *my_msg = osal_msg_allocate( len );
  my_msg[0] = CMD_JC_ZCLREQ;
  my_msg[1] = 0x00;
  my_msg[2] = SUCCESS;
  my_msg[3] = 0x00;
  my_msg[4] = len;
  my_msg[5] = aff->FrmCtrl;
  my_msg[6] = aff->DstEndPoint;
  my_msg[7] = (uint8)(aff->ClusterID & 0xff); my_msg[8] = (uint8)((aff->ClusterID >> 8) & 0xff);
  my_msg[9] = (uint8)(aff->ProfileID & 0xff); my_msg[10] = (uint8)((aff->ProfileID >> 8) & 0xff);
  my_msg[11] = aff->SrcEndPoint;
  my_msg[12] = aff->ApsCounter;
  for (int i = 0; i < aff->asduLength; i++) {
    my_msg[13+i] = aff->asdu[i];
  }
  uint8 sampleApp_task_id = *(afFindEndPointDesc(8)->task_id); // Endpoint 8 defined for SAMPLES_HA_ENDPOINT
  osal_msg_send( sampleApp_task_id, my_msg);
  // JC update end
