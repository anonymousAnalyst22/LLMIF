  ...
  if(bdbAttributes.bdbCommissioningMode & BDB_COMMISSIONING_MODE_FINDING_BINDING)
  {
    ...
    if(bdb_isDeviceNonFactoryNew())
    {
      ...
      if( bdb_CurrEpDescriptorList->epDesc->epType & BDB_FINDING_AND_BINDING_INITIATOR)  //F&B as Initiator
      {
        ...
        {
          // JC NOTES: This is the place where the Identify Query specified in BDB F&B (Figure 7) is initiated.
...
UINT16 bdb_event_loop(byte task_id, UINT16 events)
{
  ...
  {
    /**
     * JC NOTES: In this block, the device just sent a Identify Query but there is timeout waiting for the reply.
     * If FINDING_AND_BINDING_PERIODIC_ENABLE is set True, the device will try again to send Identify Query and wait for reply.
    **/ 
    if ( FINDING_AND_BINDING_PERIODIC_ENABLE == TRUE )
    {
      bdb_CurrEpDescriptor = bdb_setEpDescListToActiveEndpoint();
      ...
      if(bdb_CurrEpDescriptor != NULL) //just a safty check. The fact that we got to this functuon at all means that this cannot be NULL
      {
        ...
        if(bdb_FB_InitiatorCurrentCyclesNumber > 0)
        {
          ...
          if(bdb_getRespondentRetry(pRespondentHead) != NULL)
          {
            printf("[BDB.c:2634] Good news! there are Identify Query Response detected!\n");
          }
  ...
  if(events & BDB_RESPONDENT_PROCESS_TIMEOUT)
  {
    ...
    /**
     * JC NOTES: The BDB_RESPONDENT_PROCESS_TIMEOUT is generated when IDENTIFY_QUERY_RSP_TIMEOUT (bdb_FindingAndBinding.c: 882)
    **/
