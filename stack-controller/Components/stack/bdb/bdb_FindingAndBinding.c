...
/*********************************************************************
 * @fn      bdb_SendIdentifyQueryWithAddr
 *
 * @brief   Sends Identify query from the given endpoint
 *
 * @param   endpoint
 ...
 * @return  ZStatus_t
 */
ZStatus_t bdb_SendIdentifyQueryWithAddr(uint8 src_endpoint, uint16 shortAddr, uint8 endpoint)
{
  afAddrType_t dstAddr;
  ZStatus_t    status;
  ...
  status = zclGeneral_SendIdentifyQuery( src_endpoint, &dstAddr, TRUE, bdb_getZCLFrameCounter() ); 
 ...
 *
 * @brief   Process the Identify Query respondent list by sending Simple Descriptor request to 
 *          devices respondent in the list. Also send IEEE Addr Req to those 
 *          device for which a bind is created but IEEE addr is missing.
 *
 * @param   none
 *
 * @return  none
...
void bdb_ProcessRespondentList( void )
{
  ...
  /**
   * JC NOTES: Followings are the codes querying the device which just replied to an Identify Query Response
   * about the IEEE address or the simple device descriptor.
  **/
